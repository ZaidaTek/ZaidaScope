...
This is the *philosophical* part of this read-me, so I'll just be blunt: You can write, as a company for profit, software that uses e.g. ZDX.dll and keep that software closed-source and proprietary, but your clients using ZDX.dll must agree to using ZDX.dll according to its own license. You **cannot** distribute a modified ZDX.dll, but you may distribute the original ZDX.dll (with your proprietary program), but then you **must** say that your program uses ZDX.dll (which may simply be implied with your distribution of it) and if your users use your custom programs that use ZDX.dll, then you must first ask them to accept the terms of ZDX.dll (which may also simply be implied by use) as well (which thusly implies that your users must also accept the license terms of ZDX.dll, in order to use your custom programs based on ZDX.dll, in addition to your own custom program's license).
